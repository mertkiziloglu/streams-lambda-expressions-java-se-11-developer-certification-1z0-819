...
import static com.kiziloglumert.streamslambdas.exercises.TestData.*;
...
class FunctionalInterfacesExercise02Test {
    ...
    void searchByCategory() {
        Predicate<Product> predicate = exercise.createSearchPredicate(new FunctionalInterfacesExercise02.SearchCriteria(Category.FOOD, null, null, null));
    ...
    void searchByCategoryAndName() {
        Predicate<Product> predicate = exercise.createSearchPredicate(new FunctionalInterfacesExercise02.SearchCriteria(Category.FOOD, ".*t.*", null, null));
        assertThat(TestData.getProducts().stream().filter(predicate).collect(Collectors.toList()))
    ...
    void searchByAllCriteria() {
        Predicate<Product> predicate = exercise.createSearchPredicate(new FunctionalInterfacesExercise02.SearchCriteria(Category.OFFICE, ".*n.*", new BigDecimal("3.00"), new BigDecimal("8.00")));
        assertThat(TestData.getProducts().stream().filter(predicate).collect(Collectors.toList()))
    ...
    void emptyResult() {
        Predicate<Product> predicate = exercise.createSearchPredicate(new FunctionalInterfacesExercise02.SearchCriteria(Category.OFFICE, "Apples", null, null));
        assertThat(TestData.getProducts().stream().filter(predicate).collect(Collectors.toList()))
